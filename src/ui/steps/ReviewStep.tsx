...
export function ReviewStep({ ctx }: { ctx: WizardCtx }) {
  ...
  if (client) {
    if (simple) {
      rows.push(["Nickname", c.config.name]);
    } else {
      rows.push(
        [
          "ezQuake",
        ...
        [
          "Game data",
          o.pak1 ? "Full game (your pak1.pak)" : "Shareware episode",
        ],
  ...
  if (server) {
    if (simple) {
      rows.push(["Server name", s.hostname]);
    } else {
      rows.push(
  ...
  // Simple mode says one number — how big the download is. The per-package
  // breakdown is Advanced's business.
  if (simple) rows.push(["Download", formatBytes(plan.downloadBytes)]);

  return (
    <div className="flex flex-col gap-6">
      <KeyValue rows={rows} testId="review-summary" />
      ...
      {simple && (
        <p
          ...
          data-testid="simple-defaults"
        >
          Want to change the details?
          <Button onClick={switchToAdvanced} testId="switch-advanced">
            Switch to Advanced
          </Button>
        </p>
      )}
